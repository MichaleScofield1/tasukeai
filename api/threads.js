  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      const threads = result.rows.map(thread => ({
        ...
        tags: thread.tags,
        status: thread.status || 'open',       // デフォルト値を設定
    ...
    if (req.method === "POST") {
      ...
      // ★ フロントエンドから authorNickname が渡されているのでそれを使用
      // もし渡されていない場合のみ、DBから取得
      let nickname = authorNickname;
      let department = '';
      let year = '';

      if (!nickname) {
        // usersテーブルから取得（フォールバック）
        ...
        year = userResult.rows[0].year || '';
      }

      // DBに保存（スネークケース）
      const result = await query( 
        `INSERT INTO threads (title, content, author_id, author_nickname, tags, status)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`,
        [title, content, authorId, nickname, tags || '', 'open']
      );
